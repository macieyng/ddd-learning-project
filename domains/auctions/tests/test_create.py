...
from ..auctions import AuctionRepoInterface, UserRepoInterface, IdGeneratorInterface, UserObject, AuctionUpTime, AuctionService, AuctionDTO, AuctionStatus, AuctionObject
...
class NaiveAuctionRepo(AuctionRepoInterface):
    ...
    def create_auction(self, auction: AuctionObject):
        auction_repo_object = NaiveAuctionRepoObject.from_auction_object(auction, self.get_new_id())
...
def test_create_auction():
    ...
    user = UserObject(user_id=user_repo.get_new_id(), name="user", email="t@example.com", snapshot_id=user_repo.get_new_id())
